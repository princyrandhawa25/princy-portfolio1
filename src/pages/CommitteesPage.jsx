...
const normalizeCommittees = (payload) => {
  if (!payload) return [];
...
const CommitteesPage = () => {
  const [committees, setCommittees] = useState(fallbackCommittees);

  useEffect(() => {
    fetchData("committes.json").then((data) => {
      const normalized = normalizeCommittees(data);
      if (normalized.length) setCommittees(normalized);
    });
  }, []);

  if (!committees.length) {
    return (
      <section className="py-10 space-y-4 text-white">
  ...
  return (
    <section className="py-10 space-y-6 text-white">
      <div className="space-y-2">
        <h2 className="text-3xl font-heading">
          Academic &amp; Research Committees
        </h2>
        <p className="text-white/70 max-w-3xl">
          Selected national and international committee roles highlighting
          technical leadership, conference stewardship, and outreach.
        </p>
      </div>
